...
- 任务 ID：`<task-id>`
- Orchestrator 会话：`<date-or-session-id>`
- 目标分支：`worker/<task-slug>`
- 目标 worktree：`.worktrees/<task-slug>`
- 必须写入的报告：`.agent/reports/<task-id>.md`
- 派发时状态：`assigned`
...
使用 worker 报告模板写入 `.agent/reports/<task-id>.md`。报告必须包含：
