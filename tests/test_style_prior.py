...
class StylePriorTests(unittest.TestCase):
    ...
    def test_build_style_prior_from_training_index_writes_rgb_statistics(self):
        ...
        self.assertEqual(style_prior["schema_version"], "v0.72.3")
    ...
    def test_sample_style_policy_from_prior_writes_selected_style_artifact(self):
        ...
        self.assertEqual(policy["schema_version"], "v0.72.3")
    ...
    def test_cli_samples_style_policy(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            index_path = self.write_training_index(root)
            prior_path = root / "style_prior.json"
        ...
        self.assertIn("sampled style policy written", result.stdout)
        self.assertEqual(policy["artifact_type"], "sampled_style_policy")
        self.assertEqual(policy["sample_id"], "style-cli")
