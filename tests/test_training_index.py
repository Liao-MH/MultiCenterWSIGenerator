...
class TrainingIndexTests(unittest.TestCase):
    ...
    def test_build_training_index_writes_four_level_tile_records(self):
        ...
        self.assertEqual(summary["schema_version"], "v0.80.0")
        ...
        self.assertEqual(
            records[0]["conditioning"]["style"]["source"],
            "training_or_generation_config",
        )
        self.assertEqual(
            records[0]["conditioning"]["coord"]["cascade_level"],
            "1/32",
        )
        self.assertEqual(
            records[0]["conditioning"]["coord"]["target_image_shape"],
            [16, 16],
        )
        self.assertEqual(
            records[-1]["conditioning"]["coord"]["target_image_shape"],
            [512, 512],
        )
        self.assertEqual(
            records[0]["conditioning"]["source"]["required_when_anchor_gt_0"],
            True,
        )
        self.assertEqual(
            records[0]["conditioning"]["structure_anchor"]["policy"],
            "source_condition_required_when_anchor_gt_0",
        )
