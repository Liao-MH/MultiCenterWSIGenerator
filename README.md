...
v0.72.0 在 v0.71.0 sampled policy condition packet 集成基础上继续补齐输出审计链：smoke generation metadata、`generation_run.json`、torch diffusion smoke sample manifest 和 torch smoke generation metadata/run summary 现在会继续保留 `sampled_style_policy` / `sampled_texture_policy` 的 selected style/token 摘要、sample id、artifact path、seed、selection policy 和 limitations；缺失关键摘要字段会显式失败。不提供 sampled policy 时，既有 style seed 与 texture prior 摘要保持不变。当前能力仍是可审计摘要贯通，不是 production style encoder、texture codebook、VQ-VAE 或生成模型。
...
上述完整环境已在当前机器验证：Python `3.11.15`、PyTorch `2.12.0+cu130`、CUDA 可用、GPU 为 `NVIDIA GeForce RTX 5060 Ti`，并通过 `python -m unittest discover -s tests -v` 的 `249` 个测试。若只需要最小 CLI/schema 功能，也可以安装基础包：
