...
v0.72.4 在 v0.72.3 smoke/proxy、P4 streaming contract 和 tile blending 内存收敛基础上加固 checkpoint inference contract：`usable_for_inference=true` 现在必须同时满足 `status=trained`、非空 `training_backend` / `target_type` / `checkpoint_path` / `checkpoint_sha256`、checkpoint 文件存在且 SHA-256 匹配，并显式提供 `inference_contract.backend_type`、`artifact_role`、`production_ready` 和 `limitations`。`usable_for_inference=false` 的 skeleton / smoke checkpoint manifest 仍可加载，但不能用于 generation plan。当前能力仍是可审计契约加固，不是 production latent diffusion / ControlNet / DiT 训练或真实 production inference backend。
