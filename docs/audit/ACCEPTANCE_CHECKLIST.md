# 审计验收清单（v0.64.0 复审计，原始 v0.62.0 基线）

## 进度结论

- 当前项目处于“可测试 core/CLI 骨架 + smoke/proxy 生成验证链 + P1 契约闭环 + 独立 conda 环境验证 + P2 可交互 PySide6 配置页”阶段。
- 相比研究设计中的完整项目成果，已完成数据契约、基础 I/O、统计 prior、condition packet、smoke 级生成、基础 OME-TIFF/QC/归档、UI 控制层、可交互 PySide6 配置页和当前机器的 PyTorch/PySide6 依赖验证；尚未完成 production 级 latent diffusion/ControlNet 训练与推理、GUI 内执行/监控/输出查看、生产级 WSI 流式写出/恢复和真实 SVS 全链路复跑。
- 按设计目标拆解，M1 基本完成；M2-M4/M6 为基础能力完成但仍偏 smoke/proxy；M5 仍为 smoke/proxy；M7 已有可交互配置页但还缺 GUI 内执行与输出查看。
- 当前 v0.64.0 已提交到 `main`，尚未打 tag 或形成发布包。
...
| AC-CORE-01 | 版本号同步为 `v0.64.0` | `VERSION`、`pyproject.toml`、`src/he_wsi_generator/constants.py`、`configs/generation.default.json`、`tests/test_version.py` | 已完成 | `mamba run -n MultiCenterWSIGenerator he-wsi-gen --version` 返回 `v0.64.0` |
...
| AC-TEST-01 | 常规单元测试覆盖面较广 | `mamba run -n MultiCenterWSIGenerator python -m unittest discover -s tests -v` | 已完成 | 独立 conda 环境中通过 `206` 个测试，未再因缺少 PyTorch 或 PySide6 跳过 P2 路径 |
...
| AC-ENV-03 | 当前环境可导入 PySide6 并创建现有 UI 窗口 | `QT_QPA_PLATFORM=offscreen mamba run -n MultiCenterWSIGenerator python - <<'PY' ... create_main_window() ... PY` | 已完成 | offscreen smoke 返回窗口标题 `MultiCenterWSIGenerator`、`generation_config_path_input` 存在、保存按钮可用、widget count `126` |
...
| AC-AUDIT-01 | 本轮复审计已捕获当前工作区状态 | `git status --short`、`git log -1 --oneline`、`docs/CHANGELOG.md` Audit Snapshot | 已完成 | v0.64.0 已提交为主分支基线；尚未 tag 或打包发布 |

## 缺失

| ID | 验收项 | 文件或测试证据 | 当前状态 | 备注 |
|---|---|---|---|---|
| AC-MISS-03 | GUI 内完整执行、监控和输出查看流程 | `src/he_wsi_generator/ui/pyside_app.py`、`src/he_wsi_generator/ui/jobs.py` | 缺失 | 当前 UI 可保存配置并创建 queued job record，但不会在 GUI 内执行/监控 job 或展示 metadata/QC 输出摘要；`run-generation` 仍保持 CLI-first |
...
| ID | 验收项 | 文件或测试证据 | 当前状态 | 备注 |
|---|---|---|---|---|
| AC-DEV-01 | M7 “PySide6 控制台”表述与实现一致 | `README.md` M7 行、`docs/dev/...`、`src/he_wsi_generator/ui/pyside_app.py` | 偏离 | 单页任务配置已完成；任务执行/状态监控/输出查看仍未完成 |
...
| AC-VER-05 | v0.64.0 tag/发布包状态 | `git tag --list` 未在本轮执行发布流程 | 未验证 | v0.64.0 已提交到 `main`，但尚未 tag 或打包发布 |
| AC-VER-06 | worker 隔离区清理状态 | `git worktree list --porcelain` | 未验证 | `.worktrees/p1-qc-review-validate`、`.worktrees/p1-output-summary-contract`、`.worktrees/p2-ui-form` 与 `.worktrees/p2-ui-job-command` 仍存在；若不再需要，后续应清理 |
