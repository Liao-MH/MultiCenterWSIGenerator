# 审计验收清单（v0.72.0 复审计，原始 v0.62.0 基线）
...
- 当前 v0.72.0 作为本轮补救基线提交，尚未打 tag 或形成发布包。
...
| AC-CORE-01 | 版本号同步为 `v0.72.0` | `VERSION`、`pyproject.toml`、`src/he_wsi_generator/constants.py`、`configs/generation.default.json`、`tests/test_version.py` | 已完成 | 本轮最终验证 `he-wsi-gen --version` 返回 `v0.72.0`，包元数据/常量为 `0.72.0`、`v0.72.0`、`0.72.0` |
...
| AC-TEST-01 | 常规单元测试覆盖面较广 | `mamba run -n MultiCenterWSIGenerator python -m unittest discover -s tests -v` | 已完成 | 本轮最终验证通过，`Ran 249 tests in 15.117s OK`；覆盖 PyTorch、PySide6、P2/P4/P5 路径 |
...
| AC-AUDIT-01 | 本轮复审计已捕获当前工作区状态 | `git status --short`、`git log -1 --oneline`、`docs/CHANGELOG.md` | 已完成 | v0.72.0 作为本轮主分支补救基线；尚未 tag 或打包发布 |
...
| AC-VER-05 | v0.72.0 tag/发布包状态 | `git tag --list` 未在本轮执行发布流程 | 未验证 | v0.72.0 尚未 tag 或打包发布 |
