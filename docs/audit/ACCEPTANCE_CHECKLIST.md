# 审计验收清单（v0.65.1 复审计，原始 v0.62.0 基线）

## 进度结论

- 当前项目处于“可测试 core/CLI 骨架 + smoke/proxy 生成验证链 + P1 契约闭环 + 独立 conda 环境验证 + P2 可交互 PySide6 配置页 + GUI 内同步执行/刷新/输出摘要查看 + P6 维护性收敛”阶段。
- 相比研究设计中的完整项目成果，已完成数据契约、基础 I/O、统计 prior、condition packet、smoke 级生成、基础 OME-TIFF/QC/归档、UI 控制层、可交互 PySide6 配置页、GUI 内同步执行 queued job/刷新状态/查看输出摘要、CLI 命令分发拆分、PyTorch smoke training 纯 helper 拆分和当前机器的 PyTorch/PySide6 依赖验证；尚未完成 production 级 latent diffusion/ControlNet 训练与推理、生产级 layout/style/texture prior、生产级 WSI 流式写出/恢复和真实 SVS 全链路复跑。
- 按设计目标拆解，M1 基本完成；M2-M4/M6 为基础能力完成但仍偏 smoke/proxy；M5 仍为 smoke/proxy；M7 的本地同步 GUI flow 已补齐，后台 daemon、并发队列和运行中进程终止仍不属于当前实现；P6 的首轮行为保持维护性收敛已完成。
- 当前 v0.65.1 作为本轮补救基线提交，尚未打 tag 或形成发布包。
...
| AC-CORE-01 | 版本号同步为 `v0.65.1` | `VERSION`、`pyproject.toml`、`src/he_wsi_generator/constants.py`、`configs/generation.default.json`、`tests/test_version.py` | 已完成 | `mamba run -n MultiCenterWSIGenerator he-wsi-gen --version` 返回 `v0.65.1` |
...
| AC-AUDIT-01 | 本轮复审计已捕获当前工作区状态 | `git status --short`、`git log -1 --oneline`、`docs/CHANGELOG.md` | 已完成 | v0.65.1 作为本轮主分支基线提交；尚未 tag 或打包发布 |
...
| AC-VER-05 | v0.65.1 tag/发布包状态 | `git tag --list` 未在本轮执行发布流程 | 未验证 | v0.65.1 已提交，尚未 tag 或打包发布 |
