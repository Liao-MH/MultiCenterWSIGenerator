...
当前仓库不是完整 production H&E WSI 生成器，而是一个可测试、可追踪的 core/CLI 工程骨架，并具备 smoke/proxy 级生成、QC 和归档链路。补救顺序应先修正契约和文档诚实度，再补可交互 UI，最后进入生产模型和大规模 WSI 写出能力。
...
- 建议任务包：
  - 建立独立 UI 环境：`python -m pip install -e ".[ui,yaml]"` 或 conda 子环境。
  - 把 `pyside_app.py` 从只读字段展示升级为真实表单：路径选择、anchor preset、seed、sample steps、prior/checkpoint、输出目录、QC 设置。
  - 实现 label mapping 弹窗或最小可验收映射面板，未映射编号必须阻塞。
  - 将 UI 表单保存为 JSON/YAML，并能调用既有 CLI/core 或 local job runner。
  - 给 PySide6 交互层补最小 GUI 单元/集成测试；无 GUI 环境下保留明确 skip 或 offscreen 验证策略。
- 决策点：是否让 `run-generation` 默认先弹配置页需要用户确认；默认建议保留 CLI 自动化，把 GUI 放在 `launch-ui`。
- 完成标准：安装 PySide6 后，用户能通过 UI 配置并启动一个 smoke generation job，输出配置、状态、metadata/QC 路径均可追踪。
...
- 当前状态：已创建 conda 环境 `MultiCenterWSIGenerator`，安装完整可选依赖并通过 PyTorch/UI 定向测试、PySide6 offscreen 窗口 smoke 和全量单元测试；真实 SVS 全链路仍未在本轮复跑。
- 建议任务包：
