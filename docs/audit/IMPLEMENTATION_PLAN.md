...
- 当前状态：已创建 conda 环境 `MultiCenterWSIGenerator`，安装完整可选依赖并通过 PyTorch/UI/P4/P5 定向测试、PySide6 offscreen 表单 smoke，以及 v0.72.1 全量 `249` 个单元测试；真实 SVS smoke/proxy 复跑已在本轮完成。
- 建议任务包：
...
- 当前状态：v0.65.1 已完成首轮维护性收敛；v0.72.1 保持统计 prior 的 deterministic policy helper、condition packet 审计消费和 generation 输出摘要保留，并完成当前环境真实 SVS smoke/proxy 复跑验证，但未扩展到 production prior/model。
- 建议任务包：
