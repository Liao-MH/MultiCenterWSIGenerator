...
- 当前状态：已创建 conda 环境 `MultiCenterWSIGenerator`，安装完整可选依赖并通过 PyTorch/UI/P4/P5 定向测试、PySide6 offscreen 表单 smoke，以及 v0.72.0 全量 `249` 个单元测试；真实 SVS 全链路仍未在本轮复跑。
- 建议任务包：
...
- 当前状态：v0.65.1 已完成首轮维护性收敛；v0.72.0 增加了统计 prior 的 deterministic policy helper、condition packet 审计消费和 generation 输出摘要保留，但未扩展到 production prior/model。
- 建议任务包：
