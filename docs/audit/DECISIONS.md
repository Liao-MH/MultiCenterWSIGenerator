...
- 原始审计基线为 `v0.62.0`；当前补救开发版本为 `v0.64.0`。
- v0.64.0 允许修改 P2 可交互 PySide6 配置页相关的 `src/he_wsi_generator/ui/`、`tests/`、配置、README 和审计文档。
- 审计文件统一维护在 `docs/audit/`，不再放在 `docs/` 根目录。
- 本轮按仓库版本规则升级版本号，因为新增了向后兼容的 CLI/schema 校验入口并收紧输出摘要契约。
- 截至本次复审计，v0.64.0 已提交为 `main` 分支基线，但不是已 tag 或打包发布的 release；不能表述为已经发布。
...
当前 `v0.64.0` 不能被表述为完整项目成果，只能表述为可审计工程骨架、smoke/proxy 级验证链和可交互配置页。
...
- 不把 main 上的 v0.64.0 基线提交直接表述为已发布版本；tag、打包需另行执行并验证。
- 不把当前 PySide6 配置页描述为完整 GUI flow；它能保存配置和创建 queued job，但还不能在 GUI 内执行/监控生成或展示输出摘要。
