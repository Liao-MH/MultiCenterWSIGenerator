...
def validate_generation_config(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if non_copy:
        raise ValidationError(
            "non_copy_patch_nearest_neighbor_search must be false in v0.43.0"
        )
...
def validate_qc_report(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if patch_search:
        raise ValidationError(
            "non_copy_report.patch_nearest_neighbor_search must be false in v0.43.0"
        )
...
def _validate_annotation_record(value: Any, path: str) -> None:
    ...
    if label_encoding != "integer_index":
        raise ValidationError(f"{path}.label_encoding must be integer_index in v0.43.0")
    _ensure_mapping(
