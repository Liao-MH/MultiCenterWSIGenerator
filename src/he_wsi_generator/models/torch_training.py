...
def _diffusion_checkpoint_manifest(
    torch,
    training_index_path: str | Path,
    checkpoint_path: Path,
    checkpoint_hash: str,
...
def _validate_vae_checkpoint_manifest(manifest: dict[str, Any]) -> None:
...
def _validate_vae_checkpoint_payload(payload: Any, manifest: dict[str, Any]) -> None:
...
def _vae_diffusion_manifest_fields(diffusion: dict[str, Any]) -> dict[str, Any]:
...
def _validate_diffusion_checkpoint_manifest(
    ...
    _validate_condition_feature_schema(checkpoint_manifest.get("condition_feature_schema"))
    _validate_cross_scale_condition_schema(
        checkpoint_manifest.get("cross_scale_condition_schema")
    )
    _validate_denoiser_architecture(
    ...
    _validate_diffusion_schedule(diffusion, sample_steps)


def _validate_diffusion_checkpoint_payload(checkpoint_payload: Any) -> None:
    ...
    _validate_condition_feature_schema(checkpoint_payload.get("condition_feature_schema"))
    _validate_cross_scale_condition_schema(
    ...
    _validate_diffusion_schedule(diffusion, sample_steps=1)
    latent_shape = diffusion["latent_batch_shape"]
    latent_channels = int(latent_shape[1])
...
def _validate_diffusion_schedule(diffusion: dict[str, Any], sample_steps: int) -> None:
...
def _condition_feature_schema() -> dict[str, Any]:
    return {
...
def _cross_scale_condition_schema() -> dict[str, Any]:
    return {
...
def _validate_cross_scale_condition_schema(schema: Any) -> None:
    if not isinstance(schema, dict):
        raise TorchTrainingError("checkpoint cross_scale_condition_schema must be an object")
    expected = _cross_scale_condition_schema()
...
def _denoiser_architecture(input_channels: int, output_channels: int) -> dict[str, Any]:
    return {
...
def _validate_denoiser_architecture(
    architecture: Any,
    input_channels: Any,
    output_channels: Any,
    location: str,
    ...
    expected = _denoiser_architecture(input_channels, output_channels)
    for key, expected_value in expected.items():
...
def _validate_condition_feature_schema(schema: Any) -> None:
    if not isinstance(schema, dict):
